            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class=" main-btn-00">
                        <!-- Responsive modal -->
                        <a href="{{ route('admin.material.create') }}" class="btn btn-default waves-effect">اضافه خامة +</a>
                    </div>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table data-toggle="table" data-search="true" data-show-columns="true" data-sort-name="id" data-page-list="[8, 16, 32]" data-page-size="8" data-pagination="true" data-show-pagination-switch="true" class="table-bordered ">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$material->name}}</td>
                            <td>{{$material->status === 1 ? 'مفعل' : 'غير مفعل'}}</td>

                            <td class="actions">
                                <a href="{{ route('admin.changeStatus',[$material->status,'materials',$material->id]) }}" class="btn btn-{{$material->status == 1 ? 'secondary' : 'dark'}} waves-effect" title="الحالة"> {{$material->status == 1 ? 'إبطال' : 'تفعيل'}}</a>
                                <a href="{{ route('admin.material.edit',$material->id) }}" class="btn btn-success waves-effect" title="تعديل">تعديل</a>
                                <button type="button" class="btn btn-danger waves-effect" data-toggle="modal" data-target="#{{$material->id}}delete" title="حذف">حذف </button>
                            </td>
                        </tr>

                        <div id="{{$material->id}}delete" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
